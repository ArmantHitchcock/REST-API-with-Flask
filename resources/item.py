...
class Item(Resource):
    ...
    def post(self, name):
        ...
        item = ItemModel(name, data['price'], data['store_id']) # alternative: item = ItemModel(name, **data)
        try:
            item.save_to_db()
        except:
...
class ItemList(Resource):
    def get(self):
        return {'items': [x.json() for x in ItemModel.query.all()]}  # alternative method: return {'items': list(map(lambda x: x.json(), ItemModel.query.all()))}
